...
9. compute_formation_points ONLY:
   ...
   - Spacing must be UNIFORM: coefficient of variation of all pairwise distances must be < 0.4

10. Check pairwise distances for uniformity:
    - No clustering: no distance should be < 50% of mean distance
    - Spacing metric: CV = stdev/mean of all distances
    - If CV > 0.4, spacing is non-uniform and must be fixed
    - If >10% of distances are outliers, spacing is non-uniform
...
# ─────────────────────────────────────────────────────────────────────────────
# Post-generation runtime validator — STRICT UNIFORM SPACING CHECK
# ─────────────────────────────────────────────────────────────────────────────
...
def _runtime_validate_geometry(code: str) -> list[str]:
    ...
    Execute compute_formation_points in isolation and check:
      - Returns np.ndarray (n, 2)
      - All n rows are unique (no exact duplicates)
      - Points are UNIFORMLY SPACED (strict: coefficient of variation < 0.4)
    ...
    for n in [1, 5, 10, 15]:
        ...
        # Check for exact duplicates (within 1e-6 tolerance)
        ...
        # Compute all pairwise distances for uniformity check
        distances = []
        for i in range(n):
            for j in range(i+1, n):
        ...
        # STRICT CHECK 1: Detect clustering
        # If min distance is more than 50% smaller than mean, it's a cluster
        clustering_threshold = mean_dist * 0.5
        clustered_pairs = np.sum(distances_arr < clustering_threshold)
        total_pairs = len(distances)
        
        if clustered_pairs > 0:
            errors.append(
                f"n={n} has {clustered_pairs}/{total_pairs} clustered pairs "
                f"(min={min_dist:.4f}, mean={mean_dist:.4f}, threshold={clustering_threshold:.4f})"
            )
            continue
        
        # STRICT CHECK 2: Enforce uniform spacing using coefficient of variation
        # CV = stdev / mean. For uniform spacing, CV should be < 0.4
        cv = std_dist / mean_dist if mean_dist > 0 else 0.0
        if cv > 0.4:
            errors.append(
                f"n={n} spacing is non-uniform: CV={cv:.3f} > 0.4 "
                f"(min={min_dist:.4f}, max={max_dist:.4f}, mean={mean_dist:.4f}, stdev={std_dist:.4f})"
            )
            continue
